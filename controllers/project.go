...
func AllProjects(w http.ResponseWriter, r *http.Request) {
	...
	for _, project := range projects {
		...
		mentor_names := make([]string, 2)
		mentor_usernames := make([]string, 2)
		mentor_emails := make([]string, 2)
		...
		if project.SecondaryMentorID != nil {
			err := db.First(&secondary_mentor, project.SecondaryMentorID).Error
			if err != nil {
				w.WriteHeader(http.StatusInternalServerError)
				return
			}
			mentor_names[1] = secondary_mentor.Name
			mentor_usernames[1] = secondary_mentor.Username
			mentor_emails[1] = secondary_mentor.Email
		}
